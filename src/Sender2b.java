...
public class Sender2b {
    ...
    //Let's make an array of ACKed pkts:
    private static List acceptedACKs = new ArrayList();
        ...
        public void run(){
            ...
            while(running){
                ...
                try {
                    ...
                    //Add the accepted ACK if it has not been added:
                    if(acceptedACKs.size() > 0 || pkt == 1){
                        if(!acceptedACKs.contains(pkt)){
                            acceptedACKs.add(pkt);
                        }
                    }
                }
        ...
        */
        ...
        while(running){
            ...
            if(windowPkts.size() > 0) {
                ...
                while(windowPkts.size() > 0 && acceptedACKs.contains(fstNo)) {
                    windowPkts.remove(0);
                    times.remove(0);
                    if(windowPkts.size() > 0) {
            ...
            //In the case where times out, resend just the one packet:
            if(times.size() > 0){
                for(int i = 0; i < times.size(); i++){
                    long time = times.get(i);
                    byte[] bytes =  (byte[]) windowPkts.toArray()[i];
                    if(System.currentTimeMillis() - time >= retransmissionTime && !acceptedACKs.contains(getPktNo(bytes))){
                        times.remove(i);
                        DatagramPacket packet = new DatagramPacket(bytes, bytes.length, IPAddress, port);
                        clientSocket.send(packet);
                        //System.out.println("Timed sent: " + getPktNo(bytes));
                        times.add(i, System.currentTimeMillis());
                        if(bytes[2] == 1){
                            for(int j = 0; j < 50; j++){
                                clientSocket.send(packet);
                                lastTries+=1;
            ...
            if (none == true) {
                ...
                for(int i = 0; i < windowPkts.size(); i++){
                    ...
                    byte[] bytes =  (byte[]) windowPkts.toArray()[i];
                    //System.out.println("Sent pkt: " + getPktNo(bytes));
                    DatagramPacket packet = new DatagramPacket(bytes, bytes.length, IPAddress, port);
                    clientSocket.send(packet);
